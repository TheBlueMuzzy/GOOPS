...
export class GameEngine {
    ...
    /**
     * Handle active piece lock timing based on physics collision state.
     * Physics owns falling motion (Y position via blob.gridCells).
     * GameEngine owns lock timing and piece rotation (x stays constant relative to screenX).
     *
     * NOTE: We do NOT sync activeGoop.x here - that would break rotation.
     * The piece's grid X is determined by screenX + tankRotation when needed.
     *
     * @param physicsIsColliding - true if physics reports piece can't fall more
     * @param physicsGridY - current Y position from physics (visual Y, 0-15 range)
     */
    tickActivePiece(physicsIsColliding: boolean | null, physicsGridY?: number): void {
        if (!this.state.activeGoop) return;
        if (physicsIsColliding === null) return; // No physics blob yet

        // Sync Y position from physics (physics is source of truth during falling)
        // NOTE: Do NOT sync X - that would break piece rotation
        if (physicsGridY !== undefined) {
            // Convert visual Y (0-15) to full grid Y (with buffer)
            this.state.activeGoop.y = physicsGridY + BUFFER_HEIGHT;
        }
    ...
    /**
     * Get the fall speed for active piece (pixels per second).
     * Used by physics to determine fall rate.
     *
     * Scale conversion: Goops uses 30px cells, Proto-9 uses 50px cells
     * To match Proto-9 visually, we use cells/second as the base unit:
     *   Proto-9: 200px/s ÷ 50px = 4 cells/sec
     *   Goops:   baseCellsPerSec × 30px = pixels/sec
     *
     * Tweakable: Adjust baseCellsPerSec for desired feel (2-4 feels good)
     */
    getFallSpeed(): number {
        const baseCellsPerSec = 2; // Cells per second (tweakable: 2=slow, 4=Proto-9 speed)
        const baseFallSpeed = baseCellsPerSec * 30; // Convert to pixels/sec for 30px cells
        const denseLevel = this.powerUps['DENSE_GOOP'] || 0;
        const speedMultiplier = 1 + (denseLevel * 0.125);
        const fastDropMultiplier = this.isFastDropping ? FAST_DROP_FACTOR : 1;
        return baseFallSpeed * speedMultiplier * fastDropMultiplier;
    }
    ...
    public tick(dt: number) {
        ...
        // Active piece lock timing - called externally after physics step
        // Physics owns falling motion, GameEngine owns lock timing
