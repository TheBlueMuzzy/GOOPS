...
export interface UseSoftBodyPhysicsReturn {
  ...
  /** Register a callback to sync state before each physics step */
  setPreStepCallback: (callback: (() => void) | null) => void;
}
...
export function useSoftBodyPhysics(
  ...
  // Pre-step callback for syncing game state before physics (e.g., falling blob targets)
  // This is called at the START of each step() to ensure physics uses current state
  const preStepCallbackRef = useRef<(() => void) | null>(null);
  ...
  const step = useCallback(
    (dt: number) => {
      ...
      // Call pre-step callback to sync any state before physics runs
      // This syncs falling blob target to activeGoop position
      if (preStepCallbackRef.current) {
        preStepCallbackRef.current();
      }
  ...
  /**
   * Register a callback to sync state before each physics step.
   * This is critical for falling blobs - ensures target position is current.
   */
  const setPreStepCallback = useCallback((callback: (() => void) | null) => {
    preStepCallbackRef.current = callback;
  }, []);
